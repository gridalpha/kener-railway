...
| `/usr/bin/ping` carries the `cap_net_raw` file capability. Railway drops `CAP_NET_RAW`, and the kernel then refuses to `execve` the binary at all, so Node's `spawn` throws `EPERM` inside the monitor worker. | The capability is removed in a build layer so `ping` runs and fails cleanly. It still cannot send — Railway leaves `net.ipv4.ping_group_range` at the kernel default `1 0`, so the unprivileged ICMP socket is unavailable as well. The entrypoint measures this at boot and logs it, because Kener can only surface the result as *host is unreachable*. **Use a TCP Port, DNS or HTTP/API monitor instead of Ping.** |
